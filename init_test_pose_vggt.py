...
def apply_confidence_based_filtering(points_3d, conf_values, points_rgb, points_xyf, max_points=100000, output_dir=None):
    """
    基于置信度的智能过滤：按置信度从高到低排序，选择最佳的max_points个点
    包含详细的置信度统计和可视化
    """
    ...
    # 四分位数
    conf_q1 = np.percentile(conf_flat, 25)    # 第1四分位数 (25%)
    conf_q3 = np.percentile(conf_flat, 75)    # 第3四分位数 (75%)
    conf_iqr = conf_q3 - conf_q1              # 四分位距
